...
const OffersCarousel: React.FC<OffersCarouselProps> = ({ offers }) => {
  ...
  return (
    <div className="relative w-full max-w-screen-2xl mx-auto px-4 py-4">
      ...
      <div
        className="w-full flex gap-4 overflow-x-auto snap-x snap-mandatory pb-6 -mb-6 scrollbar-hide"
        dir="rtl"
      >
        {offers.map((offer, index) => {
          ...
          return (
            <div
              key={offer.id}
              className="snap-start flex-shrink-0 w-[88%] md:w-[48%] lg:w-[45%] xl:w-[31%]"
            >
              <Link
